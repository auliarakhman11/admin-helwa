...
class JurnalController extends Controller
{
    public function laporanKeuangan(Request $request)
    {
        ...
        $pengeluaran = Pengeluaran::select('pengeluaran.akun_id')->selectRaw('SUM(jumlah) as ttl_pengeluaran')->where('tgl', '>=', $tgl1)->where('tgl', '<=', $tgl2)->where('void', 0)->groupBy('akun_id')->with('akun')->get();

        $gapok = Gapok::select('karyawan_id')->selectRaw('SUM(jumlah) as ttl_gapok')->where('tgl', '>=', $tgl1)->where('tgl', '<=', $tgl2)->groupBy('karyawan_id')->with('karyawan')->get();
    ...
    {
        $penjualan = PenjualanKasir::where('void', 0)->where('tgl', '>=', '2026-06-01')->where('tgl', '<=', '2026-06-30')->get();
        $resep = Resep::all();
